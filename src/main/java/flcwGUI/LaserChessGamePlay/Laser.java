...
public class Laser {
    ...
    //记录激光的方向
    private final Direction[][] vec_path;
    ...
    private boolean event(Chess chess, boolean[] kill) {
        if (chess instanceof ChessKing) {
            kill[0] = true;
            return false;
        } else if (chess instanceof ChessShield chessShield) {
            //这几种情况是要杀的
            ...
            //无论如何，激光都会停止传播
            return false;
        } else if (chess instanceof ChessTwoWayMirror chessTwoWayMirror) {
            if (d == Direction.TOP) {
            ...
            //激光不会停止传播，也不会杀棋子
            return true;
        } else if (chess instanceof ChessOneWayMirror chessOneWayMirror) {
            if (d == Direction.TOP) {
